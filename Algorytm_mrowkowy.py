...
def visualize_solution(graph, solution):
    ...
    nx.draw(graph, pos, with_labels=True, node_color=colors, node_size=400)
...
class Ant:
    ...
    def colorize(self):
        ...
        for i in range(len_unvisited):
            ...
            for neighbor in self.graph.neighbors(next):
                if self.colors_assigned[neighbor] is not None:
                    tabu_colors.append(self.colors_assigned[neighbor])
    ...
    def dsat(self, node=None):
        ...
        for neighbor in self.graph.neighbors(node):
            col_neighbors.append(self.colors_assigned[neighbor])
        return len(set(col_neighbors))

    # return the pheromone trail of the pair (node,adj_node)
    def si(self, node, adj_node):
        if node < phero_matrix.shape[0] and adj_node < phero_matrix.shape[1]:
    ...
    def next_candidate(self):
        if not self.unvisited:
            ...
            for node in self.unvisited:
                heuristic_values.append((self.si(self.start, node) ** self.alpha) * (self.dsat(node) ** self.beta))
                candidates.append(node)
            max_value = max(heuristic_values)
            for i, candidate in enumerate(candidates):
                if heuristic_values[i] >= max_value:
                    candidates_available.append(candidate)
    ...
    def pheromone_trail(self):
        ...
        for i in g_nodes_int:
            for j in g_nodes_int:
                if i < phero_trail.shape[0] and j < phero_trail.shape[1]:
...
def init_pheromones(g):
    # Create a mapping from node values to indices
    node_to_index = {node: i for i, node in enumerate(g_nodes_int)}

    phero_matrix = np.ones((number_nodes, number_nodes), float)
    for node in g_nodes_int:
        for adj_node in g.neighbors(node):
            # Use the mapping to access the phero_matrix
            phero_matrix[node_to_index[node], node_to_index[adj_node]] = 0
    return phero_matrix
...
def adjacency_matrix(g):
    ...
    for node in g_nodes_int:
        for adj_node in g.neighbors(node):
            # Use the mapping to access the adj_matrix
            adj_matrix[node_to_index[node], node_to_index[adj_node]] = 1
    return adj_matrix
...
graph = create_graph("queen6.txt")

#visualize_graph(graph)

# Run the algorithm
num_colors, solution, iterations_needed = solve(graph, num_ants=10, iter=10, a=1, b=3, decay=0.8)

# Print the results
print("Number of colors used in the best solution:", num_colors)
